...
Listado.prototype.buscarRestaurante = function (id) {
    let restoBuscado = this.restaurantes.find(function (element) {
        return element.id === id;
    });

    if (restoBuscado !== undefined) {
        return restoBuscado;
    } else {
...
Listado.prototype.obtenerCiudades = function () {
    ...
    var ciudades = this.restaurantes.map(function (element) {
        return element.ubicacion;
    });

    // //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
    var ciudadesFiltradas = this.eliminarRepetidos(ciudades);
...
Listado.prototype.obtenerRubros = function () {
    var rubros = this.restaurantes.map(function (element) {
        return element.rubro;
    });

    var rubrosFiltrados = this.eliminarRepetidos(rubros);
...
Listado.prototype.obtenerHorarios = function () {
    ...
    var arregloHorariosTmp = this.restaurantes.map(function (element) {
        return element.horarios;
    });
    ...
    var arregloHorarios = [];
    arregloHorariosTmp.forEach(function (item) {
        item.forEach(function (horario) {
            arregloHorarios.push(horario);
        });
    });

    //En este arreglo vamos a poner todos los horarios pero sin repetidos
    var horariosFiltrados = this.eliminarRepetidos(arregloHorarios);
